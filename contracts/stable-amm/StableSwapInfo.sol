...
import "../stable-amm-main/SwapFlashLoan.sol";
...
contract StableSwapInfo {
    ...
    function token(address _swap) public view returns (IERC20) {
        return IERC20(address(SwapFlashLoan(_swap).getLpToken()));
    }

    function balances(address _swap) public view returns (uint256[N_COINS] memory swapBalances) {
        for (uint8 i = 0; i < N_COINS; i++) {
            swapBalances[i] = SwapFlashLoan(_swap).getTokenBalance(i);
        }
    }

    function RATES(address _swap) public view returns (uint256[N_COINS] memory swapRATES) {
        for (uint256 i = 0; i < N_COINS; i++) {
            swapRATES[i] = SwapFlashLoan(_swap).getVirtualPrice(); //Pancake has a per token rate
        }
    }

    function PRECISION_MUL(address _swap) public view returns (uint256[] memory swapPRECISION_MUL) {
        swapPRECISION_MUL = SwapFlashLoan(_swap).getTokenPrecisionMultipliers();
    }

    function _getSwapFee(SwapFlashLoan _swap) internal view returns (uint256 swapFee) {
    (,,,,swapFee,,) = _swap.swapStorage();
    }

    function _getAdminFee(SwapFlashLoan _swap) internal view returns (uint256 adminFee) {
        (,,,,,adminFee,) = _swap.swapStorage();
    }

    function calc_coins_amount(address _swap, uint256 _amount) external view returns (uint256[N_COINS] memory amounts) {
        uint256 total_supply = token(_swap).totalSupply();
        uint256[N_COINS] memory amounts;

        for (uint8 i = 0; i < N_COINS; ++i) {
            uint256 value = (SwapFlashLoan(_swap).getTokenBalance(i) * _amount) / total_supply;
            amounts[i] = value;
        }
        return amounts;
    }
    ...
    returns (uint256)
    {
        SwapFlashLoan swap = SwapFlashLoan(_swap);
        uint256[N_COINS] memory fees;
        uint256 _fee = (_getSwapFee(swap) * N_COINS) / (4 * (N_COINS - 1));
    ...
    returns (uint256[N_COINS] memory liquidityFee)
    {
        SwapFlashLoan swap = SwapFlashLoan(_swap);
        uint256 _fee = (_getSwapFee(swap) * N_COINS) / (4 * (N_COINS - 1));
        uint256 _admin_fee = _getAdminFee(swap);
        uint256 amp = swap.getA();
    ...
    function get_exchange_fee(
        ...
        uint256 dy_fee = (dy * _getSwapFee(swap)) / FEE_DENOMINATOR;

        uint256 dy_admin_fee = (dy_fee * _getAdminFee(swap)) / FEE_DENOMINATOR;
    ...
    function _xp(address _swap) internal view returns (uint256[N_COINS] memory result) {
        result = RATES(_swap);
        for (uint8 i = 0; i < N_COINS; i++) {
            result[i] = (result[i] * SwapFlashLoan(_swap).getTokenBalance(i)) / PRECISION;
        }
    }
    ...
    function _calc_withdraw_one_coin(
        ...
        uint256 amp = swap.getA();
        uint256 _fee = (_getSwapFee(swap) * N_COINS) / (4 * (N_COINS - 1));
        uint256[] memory precisions = PRECISION_MUL(_swap);
    ...
    function get_remove_liquidity_one_coin_fee(
        ...
        adminFee = (dy_fee * _getAdminFee(swap)) / FEE_DENOMINATOR;
    }
}